...
### FE Readme 및 페이지 문서화
- **README.md 최신화**
- [ ] 최신개정일, 작성자가 적혀있지 않습니다. 작성자는 `박성현, 이한경`으로 합시다.
...
- **BotServer**
- 봇 서버는 봇을 실행하는 시점에서, 벡엔드 서버에게서 모든 영문 유저 권한 이름을 가져온 후, 해당 이름으로 역할을 생성합니다. 만약 이미 해당 역할이 존재한다면, 생성을 하지 않습니다.
...
- 이때문에 앞으로 본인은오프라인 참가가 힘들 예정입니다. 참고하십쇼
